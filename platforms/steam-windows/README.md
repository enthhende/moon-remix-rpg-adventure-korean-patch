...
PowerShell 명령을 직접 입력할 필요는 없습니다. 제거는 게임을 종료한 뒤
`UNINSTALL_KO.bat`을 더블클릭합니다. 자세한 판정 문구는 패키지 안의 안내를 따르세요.

본편·엔딩·이름 조사·자료관·추가 이벤트 등의 검증은 기존 결과를 보존합니다.
실제 컨트롤러 확인 및 대학 칠판 화면은 사용자 수용 하에 **미검증(NOT_RUN)**으로
공개하는 제한입니다. 모두 통과한 것으로 표현하지 않습니다. ZIP의
`KNOWN_LIMITATIONS.json`을 확인하세요. 저장 데이터나 Cloud 설정을 자동 변경하지 않습니다.
배치파일의 정적·재현 빌드는 통과했지만 실제 Windows 더블클릭 확인은 아직
**미검증(NOT_RUN)**이며 공개 전에 짧게 확인합니다.
